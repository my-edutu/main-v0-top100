...
export function DeliveryCountry({ value = '', error, onChange }: { value?: string; error?: string; onChange?: (value: string) => void }) {
  ...
  return <div className="space-y-2">
    ...
    <select id="award-country" name="country" value={onChange ? selected : undefined} defaultValue={onChange ? undefined : selected} onChange={event => onChange?.(event.target.value)} autoComplete="country-name" required aria-invalid={!!error} aria-describedby={error ? 'award-country-error' : undefined} className="min-h-12 w-full min-w-0 border bg-white px-3">
