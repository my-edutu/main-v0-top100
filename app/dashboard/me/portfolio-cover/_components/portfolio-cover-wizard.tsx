      ...
      {!generation || generation.status === 'rejected' || generation.status === 'failed' ? (
        <div className="mx-auto max-w-xl space-y-6">
          ...
          {step === 2 && <div className="space-y-4"><h2 className="text-xl font-medium">What should appear on your cover?</h2><p className="text-sm text-neutral-500">Optional details. Leave anything blank to leave it off.</p>
            {fieldMeta.filter(item=>['name','fieldOfStudy','country','cohort'].includes(item.key)).map(({key,label,placeholder})=><div key={key} className="space-y-2"><Label htmlFor={key}>{label}{key === 'country' ? ' (optional)' : ''}</Label>{key === 'country' ? <select id={key} value={fields[key]??''} onChange={event=>setField(key,event.target.value)} className="flex h-11 w-full rounded-xl border border-input/80 bg-surface px-4 text-base shadow-xs transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"><option value="">Choose country (optional)</option>{COUNTRY_OPTIONS.map(country => <option key={country} value={country}>{country}</option>)}</select> : <Input id={key} value={fields[key]??''} onChange={event=>setField(key,event.target.value)} placeholder={placeholder} />}</div>)}
