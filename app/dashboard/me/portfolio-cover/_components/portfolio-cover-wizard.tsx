    ...
    <section className="cover-builder space-y-6" aria-labelledby="portfolio-cover-title">
      ...
      {!generation || generation.status === 'rejected' || generation.status === 'failed' ? (
        <div className="mx-auto max-w-xl space-y-6">
          ...
          {step === 3 && <div className="mx-auto w-full max-w-xl space-y-4 text-center"><h2 className="text-xl font-medium">Review before generating</h2>{preview && <img src={preview} alt="Portrait to be used" className="mx-auto h-24 w-20 rounded-xl object-cover" />}<p className="text-sm capitalize">Clothing treatment: {tailoring}</p><dl className="mx-auto w-full max-w-md divide-y text-center">{Object.entries(fields).filter(([,value])=>value).map(([key,value])=><div key={key} className="py-2"><dt className="text-xs text-neutral-500">{fieldMeta.find(item=>item.key===key)?.label.replace(' (over 5.0)','')??'Impact statement'}</dt><dd className="break-words text-sm">{value}</dd></div>)}</dl><button type="button" onClick={()=>setWelcome(true)} className="mx-auto min-h-11 text-sm underline">Review photo-edit consent</button></div>}
